...
- (void)loadWithData:(NSData*)filedata
{
    ...
    if (root) {
		...
		if (body) {
            ...
            TBXMLElement * intensive = [TBXML childElementNamed:@"intensive" parentElement:body];
            if (intensive) {
                    //NSString* countText = [TBXML valueOfAttributeNamed:@"count" forElement:pkgs];
                    //V_NSLog(@"pkgs count: %d", [countText intValue]);
                    NSMutableArray* array = [[NSMutableArray alloc] init];
                    [self loadPkgs:intensive withArray:array];
                    self.intensiveMArray = array;
                    [array release];
            }
            
            TBXMLElement * extensive = [TBXML childElementNamed:@"extensive" parentElement:body];
            if (intensive) {
                    //NSString* countText = [TBXML valueOfAttributeNamed:@"count" forElement:pkgs];
                    //V_NSLog(@"pkgs count: %d", [countText intValue]);
                    NSMutableArray* array = [[NSMutableArray alloc] init];
                    [self loadPkgs:extensive withArray:array];
                    self.extensiveMArray = array;
                    [array release];
            }
...
- (void)dealloc
{
    [self.intensiveMArray release];
    [self.extensiveMArray release];
    [super dealloc];
}
@end
